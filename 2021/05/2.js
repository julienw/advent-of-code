...
class Field {
  ...
  _plotDiagonalLine(x1, y1, x2, y2) {
    // Either both values are increasing, or one increase where the other
    // decrease.
    // Arbitrarily, let's decide x will always increase, y will either increase
    // or decrease.
    if (x1 > x2) {
      [x1, x2] = [x2, x1];
      [y1, y2] = [y2, y1];
    ...
    // It's guaranteed the diagonals are at 45° so let's be simple and increment
    // both values together.
    for (let x = x1, y = y1; x <= x2; x++, y += yDirection) {
      this._addOneInCell({ x, y });
    }
  }

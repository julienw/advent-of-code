...
class CaveMap {
  ...
  findAllPathsFromStartToEnd() {
    let currentPaths: Array<{| path: string[], smallVisited: Set<string> |}> = [
      { path: ['start'], smallVisited: new Set(['start']) },
    ];
    const result = [];

    while (currentPaths.length) {
      const newPaths = [];
      for (const { path, smallVisited } of currentPaths) {
        ...
        for (const connection of connections) {
          ...
          if (connection === 'end') {
            ...
            newPaths.push({ path: newPath, smallVisited: newSmallVisited });
          }
        }
      }

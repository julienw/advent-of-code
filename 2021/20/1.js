...
function enhance(enhancement: string, grid: string[]): string[] {
  // We need to increase the size of the starting grid first, by adding 2 pixels
  // on each side.

  const enlargedGrid = [];
  const emptyLine = '0'.repeat(grid[0].length + 4);
  enlargedGrid.push(emptyLine, emptyLine);
  for (let i = 0; i < grid.length; i++) {
    enlargedGrid.push('00' + grid[i] + '00');
  }
  enlargedGrid.push(emptyLine, emptyLine);

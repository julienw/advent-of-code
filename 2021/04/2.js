...
class State {
  ...
  numbers: number[] = [];
  boards: Board[] = [];
  winningBoards = new Set();
  previousWinningBoard = -1;
  ...
  isAllBoardWin(): boolean {
    for (const [i, board] of this.boards.entries()) {
      if (!this.winningBoards.has(i) && board.isWin()) {
        this.winningBoards.add(i);
        this.previousWinningBoard = i;
      }
    }

    return this.winningBoards.size === this.boards.length;
  }

  getLastBoardWin():
    | {| hasOneWin: false |}
    | {| hasOneWin: true, lastWinningBoard: number, score: number |} {
    if (this.previousWinningBoard < 0) {
      return { hasOneWin: false };
    }

    return {
      hasOneWin: true,
      lastWinningBoard: this.previousWinningBoard,
      score:
        this.previousNumber *
        this.boards[this.previousWinningBoard].sumOfUnpicked(),
    };
  }
...
async function run() {
  ...
  while (true) {
    ...
    if (state.isAllBoardWin()) {
      const result = state.getLastBoardWin();
      printResult(result);
      break;
    }
  }
